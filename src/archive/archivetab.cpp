...
void ArchiveTab::refreshTable()
{
    ...
    for(const auto& p:playlist){const bool has=map.contains(p.itemKey);const auto c=has?map[p.itemKey]:archive::CanonicalItem{};const auto status=archive::derivedStatus(p,has?&c:nullptr);if(status=="Protected")++protectedCount;if(status.contains("Needs")||status=="Missing"||status=="Failed"||status=="Interrupted")++needs;if(p.availability!="public")++unavailable;if(p.membership=="removed")++removed;
        ...
        const int r=m_table->rowCount();m_table->insertRow(r);auto* pos=new QTableWidgetItem(p.position<0?QStringLiteral("-"):QString::number(p.position));pos->setData(Qt::UserRole,p.entryKey);pos->setData(Qt::UserRole+1,p.itemKey);m_table->setItem(r,0,pos);m_table->setItem(r,1,new QTableWidgetItem(p.title));m_table->setItem(r,2,new QTableWidgetItem(p.availability));m_table->setItem(r,3,new QTableWidgetItem(has?c.video.state:"missing"));m_table->setItem(r,4,new QTableWidgetItem(has?c.audio.state:"missing"));m_table->setItem(r,5,new QTableWidgetItem(status));
    }
...
{
    if(!m_ready||!archive::ui::rootAvailable(m_root))return;
    const auto source=selectedSource();const auto key=selectedItemKey();const auto entryKey=selectedEntryKey();archive::Store store{archive::Paths(m_root)};
    ...
    for(const auto& line:history.split('\n')){
        ...
        QJsonParseError pe;const auto doc=QJsonDocument::fromJson(line.toUtf8(),&pe);
        if(pe.error==QJsonParseError::NoError&&doc.isObject()){
            const auto object=doc.object();const auto eventEntry=object.value("entry_key").toString();
            if((!eventEntry.isEmpty()&&eventEntry==entryKey)||(eventEntry.isEmpty()&&object.value("item_key").toString()==key))matching<<line;
        }
    }
