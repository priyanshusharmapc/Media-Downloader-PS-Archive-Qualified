...
int main(int argc,char** argv){QCoreApplication app(argc,argv);if(argc!=2)return 2;const QString name=app.arguments().at(1);try{
 ...
 else if(name=="discovery-null"){
  // Supply valid identity so the new admission gate cannot mask this older
  // null-entry guard. A paired complete response proves the fixture is usable.
  Source s;s.key="PLAUDIT";
  require(PlaylistDiscovery::parse(s,R"({"id":"PLAUDIT","entries":[]})","",0).complete,"valid discovery control rejected");
  auto r=PlaylistDiscovery::parse(s,R"({"id":"PLAUDIT","entries":[null]})","",0);
  require(!r.complete,"null entries must prevent removal inference");
  require(!r.error.contains("identity"),"identity gate masked null-entry coverage");
 }
 else if(name=="discovery-count"){
  Source s;s.key="PLAUDIT";
  require(PlaylistDiscovery::parse(s,R"({"id":"PLAUDIT","entries":[],"playlist_count":0})","",0).complete,"matching count control rejected");
  auto r=PlaylistDiscovery::parse(s,R"({"id":"PLAUDIT","entries":[],"playlist_count":3})","",0);
  require(!r.complete,"truncated playlist must be partial");
  require(!r.error.contains("identity"),"identity gate masked count coverage");
 }
 else if(name=="discovery-error"){
  Source s;s.key="PLAUDIT";const QByteArray payload=R"({"id":"PLAUDIT","entries":[]})";
  require(PlaylistDiscovery::parse(s,payload,"",0).complete,"successful process control rejected");
  auto r=PlaylistDiscovery::parse(s,payload,"ERROR: failed to fetch page",0);
  require(!r.complete,"ignore-errors cannot authorize removals");
  require(!r.error.contains("identity"),"identity gate masked process-error coverage");
 }
